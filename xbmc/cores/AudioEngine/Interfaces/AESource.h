...
public:
  ...
  /*
    The source does NOT have to honour anything in the format struct or the device
  ...
  */
  ...
  /*
    Deinitialize the source for destruction
  */
  ...
  */
  ...
  /*!
   * @brief Gets captured packets, this routine MUST block or sleep.
   ...
   * @return number of frames consumed by the source
  */
  virtual unsigned int GetPackets(uint8_t **data, unsigned int frames, unsigned int offset) = 0;


  /*!
   * @brief Return a timestamped status structure with delay and source info
   * @param status structure filled with source status
  */
  ...
  /*
    Indicates if source can handle volume control.
  */
