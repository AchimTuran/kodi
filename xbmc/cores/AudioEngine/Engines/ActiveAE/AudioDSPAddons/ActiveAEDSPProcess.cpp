 ...
 : m_streamId(streamId)
{
  ...
  m_streamTypeUsed          = AUDIODSP_ADDON_ASTREAM_INVALID;
  m_NewStreamType           = AUDIODSP_ADDON_ASTREAM_INVALID;
  ...
  for (int i = 0; i < AUDIODSP_ADDON_CH_MAX; ++i)
  {
...
bool CActiveAEDSPProcess::Create(const AEAudioFormat &inputFormat, const AEAudioFormat &outputFormat, bool upmix, bool bypassDSP, AEQuality quality, AUDIODSP_ADDON_STREAMTYPE iStreamType,
                                 enum AVMatrixEncoding matrix_encoding, enum AVAudioServiceType audio_service_type, int profile)
{
  ...
  if (iStreamType == AUDIODSP_ADDON_ASTREAM_AUTO)
    m_streamTypeUsed = m_streamTypeDetected;
  else if (iStreamType > AUDIODSP_ADDON_ASTREAM_INVALID && iStreamType < AUDIODSP_ADDON_ASTREAM_AUTO)
  ...
  else
  {
    ...
    m_streamTypeUsed = AUDIODSP_ADDON_ASTREAM_AUTO;
  }
  ...
  if (g_application.m_pPlayer->GetAudioStreamCount() > 0)
  {
    ...
    m_addonStreamProperties.strStreamName       = info.name.c_str();
    m_addonStreamProperties.strStreamLanguage   = info.language.c_str();
    m_addonStreamProperties.strStreamCodecId    = info.audioCodecName.c_str();
  }
  else
  {
    m_addonStreamProperties.strStreamName       = "Unknown";
    m_addonStreamProperties.strStreamLanguage   = "";
    m_addonStreamProperties.strStreamCodecId    = "";
  }
   ...
   */
  //m_addonSettings.uiStreamID               = m_streamId;
   ...
   */
  for (int ii = 0; ii < AUDIODSP_ADDON_CH_MAX; ii++)
  {
  ...
  if (CLog::GetLogLevel() == LOGDEBUG) // Speed improve
  {
    ...
    //                                                     "Unknown");
    CLog::Log(LOGDEBUG, "  | Name                 : %s", m_addonStreamProperties.strStreamName);
    CLog::Log(LOGDEBUG, "  | Language             : %s", m_addonStreamProperties.strStreamLanguage);
    CLog::Log(LOGDEBUG, "  | Codec                : %s", m_addonStreamProperties.strStreamCodecId);
...
AUDIODSP_ADDON_STREAMTYPE CActiveAEDSPProcess::DetectStreamType(const CFileItem *item)
{
  AUDIODSP_ADDON_STREAMTYPE detected = AUDIODSP_ADDON_ASTREAM_AUTO;
  if (item->HasMusicInfoTag())
    detected = AUDIODSP_ADDON_ASTREAM_MUSIC;
  else if (item->HasVideoInfoTag() || g_application.m_pPlayer->HasVideo())
    detected = AUDIODSP_ADDON_ASTREAM_MOVIE;
...
//      detected = AE_DSP_ASTREAM_PHONE;
    ...
    detected = AUDIODSP_ADDON_ASTREAM_AUTO;
...
{
  return 0;// m_addonSettings.iProcessSamplerate;
}
...
AUDIODSP_ADDON_STREAMTYPE CActiveAEDSPProcess::GetDetectedStreamType()
{
  return m_streamTypeDetected;
}

AUDIODSP_ADDON_STREAMTYPE CActiveAEDSPProcess::GetUsedStreamType()
{
  return m_streamTypeUsed;
}

AUDIODSP_ADDON_BASETYPE CActiveAEDSPProcess::GetBaseType(AUDIODSP_ADDON_STREAM_PROPERTIES *props)
{
  if (!strcmp(props->strStreamCodecId, "ac3"))
    return AUDIODSP_ADDON_ABASE_AC3;
  else if (!strcmp(props->strStreamCodecId, "eac3"))
    return AUDIODSP_ADDON_ABASE_EAC3;
  else if (!strcmp(props->strStreamCodecId, "dca") || !strcmp(props->strStreamCodecId, "dts"))
    return AUDIODSP_ADDON_ABASE_DTS;
  else if (!strcmp(props->strStreamCodecId, "dtshd_hra"))
    return AUDIODSP_ADDON_ABASE_DTSHD_HRA;
  else if (!strcmp(props->strStreamCodecId, "dtshd_ma"))
    return AUDIODSP_ADDON_ABASE_DTSHD_MA;
  else if (!strcmp(props->strStreamCodecId, "truehd"))
    return AUDIODSP_ADDON_ABASE_TRUEHD;
  else if (!strcmp(props->strStreamCodecId, "mlp"))
    return AUDIODSP_ADDON_ABASE_MLP;
  else if (!strcmp(props->strStreamCodecId, "flac"))
...
AUDIODSP_ADDON_BASETYPE CActiveAEDSPProcess::GetUsedBaseType()
{
...
//}
//
//bool CActiveAEDSPProcess::GetMasterModeTypeInformation(AUDIODSP_ADDON_STREAMTYPE &streamTypeUsed, AE_DSP_BASETYPE &baseType, int &iModeID)
//{
//  streamTypeUsed  = (AUDIODSP_ADDON_STREAMTYPE)m_addonStreamProperties.StreamType;
//
//  if (m_activeMode < 0)
//    return false;
//
...
const char *CActiveAEDSPProcess::GetStreamTypeName(AUDIODSP_ADDON_STREAMTYPE iStreamType)
{
  return iStreamType == AUDIODSP_ADDON_ASTREAM_MUSIC   ? "Music"     :
         iStreamType == AUDIODSP_ADDON_ASTREAM_MOVIE   ? "Movie"     :
         iStreamType == AUDIODSP_ADDON_ASTREAM_GAME    ? "Game"      :
         iStreamType == AUDIODSP_ADDON_ASTREAM_APP     ? "App"       :
         iStreamType == AUDIODSP_ADDON_ASTREAM_PHONE   ? "Phone"     :
         iStreamType == AUDIODSP_ADDON_ASTREAM_MESSAGE ? "Message"   :
         iStreamType == AUDIODSP_ADDON_ASTREAM_AUTO    ? "Automatic" :
         "Unknown";
}

bool CActiveAEDSPProcess::MasterModeChange(int iModeID, AUDIODSP_ADDON_STREAMTYPE iStreamType)
{
  bool bReturn           = false;
  ...
  //{
  //  AUDIODSP_ADDON_STREAMTYPE old = m_streamTypeUsed;
  //  CLog::Log(LOGDEBUG, "  ----  Input stream  ----");
  ...
  //                                  mode->AddonModeName().c_str(),
  //                                  GetStreamTypeName((AUDIODSP_ADDON_STREAMTYPE)m_addonStreamProperties.StreamType),
  //                                  m_addons_MasterProc[ptr].pAddon->GetAudioDSPName().c_str());
...
void CActiveAEDSPProcess::SetFFMpegDSPProcessorArray(float *array_ffmpeg[AUDIODSP_ADDON_CH_MAX], float *array_dsp[AUDIODSP_ADDON_CH_MAX], int idx[AE_CH_MAX], unsigned long ChannelFlags)
{
  /*!
  ...
  /*! @warning If you change or add new channel enums to AEChannel in 
    * xbmc/cores/AudioEngine/Utils/AEChannelData.h you have to adapt this loop
    */
  memset(array_ffmpeg, 0, sizeof(float*)*AUDIODSP_ADDON_CH_MAX);
  for (int ii = 0; ii < AUDIODSP_ADDON_CH_MAX; ii++)
  {
...
//void CActiveAEDSPProcess::GetAvailableMasterModes(AUDIODSP_ADDON_STREAMTYPE streamType, std::vector<CActiveAEDSPModePtr> &modes)
//{
//  CSingleLock lock(m_critSection);
//
...
//bool CActiveAEDSPProcess::SetMasterMode(AUDIODSP_ADDON_STREAMTYPE streamType, int iModeID, bool bSwitchStreamType)
//{
...
bool CActiveAEDSPProcess::IsMenuHookModeActive(AUDIODSP_MENU_HOOK_CAT category, int iAddonId, unsigned int iModeNumber)
{
