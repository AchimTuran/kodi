...
void CGUIDialogSubtitleSettings::OnSettingChanged(const CSetting *setting)
{
  if (setting == NULL)
    return;
...
void CGUIDialogSubtitleSettings::OnSettingAction(const CSetting *setting)
{
  if (setting == NULL)
    return;
...
void CGUIDialogSubtitleSettings::InitializeSettings()
{
  ...
  CSettingCategory *category = AddCategory("subtitlesettings", -1);
  if (category == NULL)
  {
  ...
  // subtitle stream setting
  AddButton(groupSaveAsDefault, SETTING_SUBTITLE_MAKE_DEFAULT, 12376, 0);
}
...
void CGUIDialogSubtitleSettings::AddSubtitleStreams(CSettingGroup *group, const std::string &settingId)
{
...
void CGUIDialogSubtitleSettings::SubtitleStreamsOptionFiller(const CSetting *setting, std::vector< std::pair<std::string, int> > &list, int &current, void *data)
{
...
std::string CGUIDialogSubtitleSettings::SettingFormatterDelay(const CSettingControlSlider *control, const CVariant &value, const CVariant &minimum, const CVariant &step, const CVariant &maximum)
{
  if (!value.isDouble())
    return "";
