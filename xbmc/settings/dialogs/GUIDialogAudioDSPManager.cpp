...
void CGUIDialogAudioDSPManager::Update()
{
  ...
  //    items.Add(item);
  //
  //    AUDIODSP_MENU_HOOK_CAT menuHook = helper_GetMenuHookCategory(iModeType);
  //    int continuesNo = 1;
...
DSPErrorCode_t CGUIDialogAudioDSPManager::EnableNodeCallback(uint64_t ID, uint32_t Position)
{
...
DSPErrorCode_t CGUIDialogAudioDSPManager::DisableNodeCallback(uint64_t ID)
{
  //CSingleLock lock(m_nodeCallbackLock);
...
{
  int iType = 0;// AE_DSP_MODE_TYPE_UNDEFINED;
...
CFileItem *CGUIDialogAudioDSPManager::helper_CreateModeListItem(CActiveAEDSPModePtr &ModePointer, AUDIODSP_MENU_HOOK_CAT &MenuHook, int *ContinuesNo)
{
...
int CGUIDialogAudioDSPManager::helper_GetDialogId(CActiveAEDSPModePtr &ModePointer, AUDIODSP_MENU_HOOK_CAT &MenuHook, AE_DSP_ADDON &Addon, std::string AddonName)
{
  int dialogId = 0;
  ...
  if (ModePointer->HasSettingsDialog())
  {
    ...
    if (dialogId == 0)
      CLog::Log(LOGERROR, "DSP Dialog Manager - %s - Marked settings dialog of mode %s for addon %s not found",
                            __FUNCTION__,
...
AUDIODSP_MENU_HOOK_CAT CGUIDialogAudioDSPManager::helper_GetMenuHookCategory(int CurrentType)
{
  AUDIODSP_MENU_HOOK_CAT menuHook;// = AE_DSP_MENUHOOK_ALL;
  switch (CurrentType)
  {
