...
bool CGUIFeatureButton::DoPrompt(const std::string& strPrompt, const std::string& strWarn, const std::string& strFeature, CEvent& waitEvent)
{
  ...
  if (!HasFocus())
  {
    ...
    CServiceBroker::GetApplicationMessenger().SendGUIMessage(msgFocus, WINDOW_INVALID, false);
  }
  ...
  for (unsigned int i = 0; i < COUNTDOWN_DURATION_SEC; i++)
  {
    ...
    CServiceBroker::GetApplicationMessenger().SendGUIMessage(msgLabel, WINDOW_INVALID, false);
